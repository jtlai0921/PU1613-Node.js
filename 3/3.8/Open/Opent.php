...
class Core_Open_Opent
{
     ...
     * oauth授權之後的回調頁面
     * 傳回包括 oauth_token 和oauth_token_secret的key/value陣列
     */
    ...
    {
        $response = preg_replace('/[^\x20-\xff]*/', "", $response); //清除不可見字元
        ...
        switch($ret)
        {
            ...
            case 3:
                ...
                break;
            case 4:
               // $errcode = (!in_array($errcode,array(0,4,5,6,8,9,10,11,12,13,109)) || $errcode<0) ? -1 : $errcode;//不在指定清單裡的errorcode 為非預期錯誤 預設-1
            	if($errcode==0)
            	{
            		return $jsonArr;
