...
exports["General tests"] = {
    ...
    "Set object header": function(test){
        ...
        var testObj = {
                stringValue: "String with unicode symbols: OAOUŽŠ",
                arrayValue: ["hello OAOU", 12345],
                objectValue: {
                    customerId: "12345"
                }
            };
...
exports["Text encodings"] = {
    "Punycode": function(test){
        ...
        test.equal(toPunycode("andris@age.ee"), "andris@xn--ge-uia.ee");
        test.done();
    },
    ...
    "Mime words": function(test){
        ...
        // simple
        test.equal(mc._encodeMimeWord("ass"), "=?UTF-8?Q?=C3=A4ss?=");
        test.equal(mc._encodeMimeWord("ass","B"), "=?UTF-8?B?"+(new Buffer("ass","utf-8").toString("base64"))+"?=");
        
        //multiliple
        test.equal(mc._encodeMimeWord("ass tekst on see siin voi kuidas?","Q", 20), "=?UTF-8?Q?=C3=A4ss?= tekst on see siin =?UTF-8?Q?v=C3=B5i?= kuidas?");
    ...
    "Addresses": function(test){
        var mc = new MailComposer();
        mc.setMessageOption({
            sender: '"Jaanuar Veebruar, Marts" <marts@marts.eu>'
        });
        ...
        mc.setMessageOption({
            sender: '<aavik@marts.eu>, juulius@node.ee, "Node, Master" <node@node.ee>'
        });
        
        test.equal(mc._message.from, 'aavik@xn--mrts-loa.eu, juulius@node.ee, "Node, Master" <node@node.ee>');
        
        mc.setMessageOption({
            sender: ['<aavik@marts.eu>', 'juulius@node.ee, "Node, Master" <node@node.ee>', 'andris@node.ee']
        });

        mc.setMessageOption({
            to: ['<aavik@marts.eu>', 'juulius@node.ee, "Node, Master" <node@node.ee>', 'andris@node.ee']
        });
...
exports["Mail related"] = {
    "Envelope": function(test){
        ...
        mc.setMessageOption({
            sender: '"Jaanuar Veebruar, Marts" <marts@marts.eu>',
            to: '<aavik@marts.eu>, juulius@node.ee',
            cc: '"Node, Master" <node@node.ee>'
        });

        test.deepEqual(mc._envelope, {from:[ 'marts@xn--mrts-loa.eu' ],to:[ 'aavik@xn--mrts-loa.eu', 'juulius@node.ee'], cc:['node@node.ee' ]});
        test.done();
    },
    ...
    "User defined envelope": function(test){
        ...
        mc.setMessageOption({
            ...
            envelope: {
                ...
                to: ["Andris <andris@tr.ee>, Node <andris@node.ee>", "aavik@marts.eu", "juulius@gmail.com"],
                cc: "trips@node.ee"
            },
            to: '<aavik@marts.eu>, juulius@node.ee',
            cc: '"Node, Master" <node@node.ee>'
    ...
    "Generate envelope": function(test){
        var mc = new MailComposer();
        mc.setMessageOption({
            sender: '"Jaanuar Veebruar, Marts" <marts@marts.eu>, karu@ahven.ee',
            to: '<aavik@marts.eu>, juulius@node.ee',
            cc: '"Node, Master" <node@node.ee>'
        });
        
        test.deepEqual(mc.getEnvelope(), {from: 'marts@xn--mrts-loa.eu',to:[ 'aavik@xn--mrts-loa.eu', 'juulius@node.ee', 'node@node.ee' ], stamp: 'Postage paid, Par Avion'});
        test.done();
    },
...
exports["Stream parser"] = {
    ...
    "Non UTF-8 charset": function(test){
        var mc = new MailComposer({charset: "iso-8859-1"}),
            message = "",
            mp = new MailParser(),
            subject = "Jogeva maakond, Arni kula",
            text = "Loaopold Jaager Susi",
            fromName = "Mager Moksi",
